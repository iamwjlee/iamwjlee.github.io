...
- 외상
- 대지강타(GroundSlam) - 무자비 보조 - 출혈확률 보조(공격속도증가)
- 징벌(SkillGem Smite)
    - 선대의 부름보조 - 무자비보조(출혈확률 보조)
- 선대의 수호자(AncestralProtector 토템) - 힘줄 절단
- 도약강타 - 공격속도증가
- 산산조각 - 무자비보조(근접전투) - 출혈확률 보조 - 근접물리피해
- 전쟁 깃발, 강철피부
- 여기까지 액트 1장
- 선대의 함성(AncestralCry) 뼈박살에 활용
- 순수의 전령/피의 격노
- 여기까지 액트 2장
- 결의(Determination 방어도증가 오라),전쟁깃발(오라),순수의전령(오라)
- 오라 사용에 마나가 문제가 되면, 산산조각의 근접물리피해대신 생명력 전환으로 교체!
- 선대의 대전사(AncestralWarchief,싸움 우두머리  피해증가토템)
- 뼈박살사용하기위해 패시브 노드 '부족의 광분' 과 공격숙련에 '주변의 적1명'
- 여기까지 액트 3장
- 다중 토템 보조 
    - 4링크 선대의 수호자/대전사 - 다중토템 - 힘줄절단/마무리타격
- 여기까지 액트 4장 이후에는 스킬젬 얻는 퀘스트가 없다!
- 저거넛 견인불발: 1초마다 지난 10초동안의 피격에서 예방한 물리피해의 1.5%를 생명력으로 재생
    - Prevented Physical Damage(예방한 물리 피해)
    - '물리피해를 많이 막아낼수록, 초당 피가 더 많이 차는" 생존기제
    - 출혈이나 물리지속피해는 포함되지 않는다! 
    - **불멸의 외침**(ImmortalCall) : 피해 감폭, 기본 지속시간 1초, 인내충전 소모로 지속시간 20%증가, 다른 수호스킬과 재사용 대기시간을 공유 
    - 수호 스킬(Guard Skills)은 짤은 시간동안 받는 피해를 감폭, 생존력을 극대화
        - 대표적인 스킬로는 '**강철피부**','**용암방패**','**불멸의 외침**'이며,주로 '피격시 시전보조' (소모 및 점유 배율: 250% ? 강철피부 소모마나의 2.5배가 점유)젬과 연동하여 사용한다.

- '타누 아히' 고유 건들릿 : 점유되지 않은 생명력이 차서 흡수가 제거되면 10%확률로 맹공 4초동안
    - 피흡(Leech)으로 피가 다 찼을때 발생하는 효과
    - 생명력흡수(LifeLeech) 오라는?
        - 패시브에서 '공격피해의 #%를 생명력으로 흡수' 옵션 노드
        - 생명력 흡수 보조젬
        - 몬스터에 따라 '흡수 저항'을 가질수 있어
        - 명중(hit)피해만, 점화나 출혈 같은 지속피해(DoT:Damage over Time)는 생명력흡수안돼!
        - DoT : 상태이상(점화,중독,출혈)이나 특정기술(정의의 화염)
- '베루소의 공성추' 건틀릿 : 인내 충전 하나당 근접피해 4%
- '키카자루'반지 : 1레벨당 생명력 3재생
- '학살 심장' 목걸이 : 초반에
- '영원한 투쟁' 목걸이 
...
|가. 생명력 5000| 나. 생명력 5000,막기확률 50% | 다. 생명력 5000, 피해감폭 50% |
|---|---|---|
|보통 | 중간| 굿| 

- '폭풍의 방패' 젬, '**유연한 전투원**' 노드등 사용
- '**풀블럭**'은 공격및 주문 막기확률을 모두 75%맞추는 것을 의미 한다
- '**육체와 돌**': 피해증폭및 감폭 
- '**한기의 방어구**':정지상태일때 물리,화염피해 감폭, 지속시간 2.5~4.4초
- '**운명의 저항 파우아 목걸이**':잃어버린 생명력의 일부를 피격될시 다시 회복(20%)하고 피격된다
- **고통 무시(Defy Pain)** 버서커 전직노드,10초마다 상실한 생명력 획득


[평아의 글래디 CWS 무자본](https://www.youtube.com/watch?v=UAO4uxFCQ68)
- [pob](https://pobb.in/41J5fmeyqjok)
- Level 100 Firestorm Gladiator

        상의 - 화염폭풍-원소집중-강화보조-효과집중-제어된파괴
            -CWS초반에는 원소집중 빼시고 생명력 전환 다세요
        장화 - CWS-정화의불길-청소부의시체폭발-생명력전환
        장갑 - CWS-화염쇄도-불멸의외침-생명력 전환
        방패 - 원소의순수함-폭풍의방패-활력
        뚜껑 - 저주계열 기본 원소약화-인화성-CWS 걸어주시고 초반에는 생명력전환
        목걸이 추가저주다시고 원소약화-인화성 2개중 생전 다시고 응징 추가
        무기 - 본인이 원하는 이동기쓰세요 서리점멸,방패돌진(생명력전환다세요)
        추후에 생전 전부 달면 본인에 맞게 커스텀하세요
        스펙업후 생명력회복 여유있으면 RF추가해서 딜40퍼 증폭 가져가세요
        반사 저주 원소상태이상 전부 면역에 주얼에서 타피 반지에서 출혈만 면역챙겨주시면
        모든 상태이상 다 쳐낼수있습니다
        기절은 회복이 빨라서 무시됩니다

        - 판테온은 아라칼리(지속뎀감소) 
        - 보조는 원하는거 챙기는데 저는 유굴 좋아해서 유굴씁니다(반사면역+저주면역)



- **아자디 문양** : 스발린과비교
- **불변의 힘**(진홍색 주얼) - 기절및 막기 회복 974% 증가
- **발리륨** 기절한계치가 생명력이 아닌 에너지 보막에 비례
- **고상한 오만 무궁한 주얼** 카스피로( 허영의 정점 능력치요구사항 무시,영원한 젊음 노드에 장착) 
- **도플갱어 위장** 물리,카오스피해 감폭 40퍼,6링 무조건(대충 4딥)
- **운명의 저항** 목걸이 '상실한 점유되지 않은 생명력' 점유생명력높은걸로,성유는 멸망의속삭임(금,금,금)
- **원한의 품** 반지 매우비싸! '맹약 블볼'에서도 사용
  
- 글래디는 막기에 행운적용이 전직노드에 기본적으로 채용 구지 **스발린**이 불필요  
- 이 행운적용이 아자디로 3번 돌려 
- 저항성소 벨트(약10디바인)
- **갈매기** 가면: 성소 버프효과 증가/지속, **저항성소벨트**와 함께 사용하면 저항 97%
- **원소의 순수함**사용하여 일반저항은 안맞추어, 카오스는 챙겨야
    - 모든 저항30%증가및 상태이상에 면역
- 성운 무기구매   주문피해,화염피해  
- 저거넛의 히네코라 시폭딜(치프틴 전직노드?)은 절대적


[아자디문양 상시유지](https://www.youtube.com/watch?v=qmPi5ugFElo)
[아자디문양 피부찢기 글래디](https://www.youtube.com/watch?v=t0xyGBGu2PM)
- **리슬라사의 감개** 허리고유
- **도끼마 잭** 한손도끼고유
- **아자디 문양**
- **몰려오는 폭풍** 고유각반
- 젬 실혈의 피부찢기(DPS 2만) - 포악함보조 - 방어상승보조 - 일촉즉발 보조 - 근접 물리피해 보조 - 잔혹 보조
- **타뷸라 라사** 고유로브
- 젬 영원한 축복보조(빨) - 자부심(빨) : 혈마법을 찍으면 꽁짜로 자부심을 쓸수 있다!(확인)
- 젬 방패돌진 - 인내의 함성 - 공격속도증가 보조 - 긴급명령보조 -
- 젬 포탈-사망시시전
- 피의격노-서리점멸-전쟁깃발-취약성
- 노련한보복보조 - 내장적출 - 근접물리피해 보조 - 힘줄절단 보조
- 성역제외 모든 콘텐츠 가능!
- 패시브 설명 13:09/14:19



### 글래디 CWS(Cast When Stunned) '기절 시 시전' 빌드란
- 적에게 맞아 기절하는 것을 오히려 기회로 삼아 자동으로 강력한 주문을 시전하는 메커니즘
- 공격없이 걷기만해도 사냥이돼 


### CWS

        CWS = 기절 시 시전 보조 = 기절 하면 연결된 주문이 시전(발동) 된다.
        코어 아이템
        발리륨 = 이유: 에너지 보호막 숙련에서 사라진 에너지 보호막 기절을 이 반지로 사용
        하여 에너지 보호막이 높으면 좀 곤란함(2~3천 단위). 낮으면 낮을 수록 좋음.
        피의 오늬 - 맞으면 맞은 만큼 찬다
        불변의 힘 - 기절에 걸리지만 최대한 빠르게 회복하여 걸렸나? 란 기분을 느끼게 해준다
        나머지는 모두 생존기재의 아이템
        정확한 기재는 아니지만 대충 이해하기 쉽게
        회생 노드 - 맞으면 맞은 만큼 찬다
        운명의 저항 - 맞으면 맞은 만큼 찬다
        피석화 - 맞으면 덜 아프게 맞는다
        화염의 망토 - poe의 모든 사망원인 1순위인 물리 피해를 화염 피해로 전환 - 덜 아프게 맞는다
        즉 cws 빌드란
        나는 덜 아프게 맞는데 맞는 만큼 발리륨을 이용한 무한 기절로 주문을 시전하여 적을 죽이는 빌드
        피석화 = 받는 데미지의 40퍼를 4초간 잃게 만듬
        피의 오늬 = 기절피격의 40-60퍼를 회복
        불변 = 기절 회복
        이러면 피의 오늬 맥롤 기준 100의 데미지를 받으면 피석화로 60의 데미지만 즉시 받고 40을 4초 동안 받는데 이때 60의 데미지는 즉시 회복하고 40의 데미지만 4초동안 받게 되서 1방에 터지지 않는 다면 거의 무적임
        피석화 데미지 때문에회생이랑 재생을 땡기는 거임.



### 결전 글레디/금고,균열 - 저거넛으로
### 저거넛의 장점 - 히네코라 
- 금고나 균열에서 히네코라 시폭(시체폭발)딜은 절대적   
- 방패는 스발린을 구매해야  


[까까모리의 균열런 파밍영상](https://www.youtube.com/watch?v=QWtp__qk5pE)
- 사구 지도 
- 아이템 수량 100% 80%
- 지역이 태초자의 기억의영향을 받음 - 지도
- 양질의 환영으 오브 2~3개 발라
