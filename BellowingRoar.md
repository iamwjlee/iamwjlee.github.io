...
[푸들멈머의 패스오브엑자일2 소서리스 화염구](https://www.youtube.com/watch?v=v4AoH7jiiCs)
    ...
    대마법사: 명상(추가정신력10소모)+번개숙련
    태양의보주/화염벽: 과잉,번개노출,화염노출
    서리방벽: 연속주문+고드름+빙하작용+밀려오는 폭풍+ 냉기숙련

    점멸(정신력 60소모)
    전도성(대마법사 사용시): 저주받은 지대+ 조고된저주+광장 
    인화성(대마법사 미사용시)
    힘의부적(보스전에서)
    격노의유령(화염구랑 쓰면 프레임드랍문제)
    마나가 많이 부족하면 주로 사용하는 기술에 (스킬비용40%감폭)이나 영혼고갈(마나회복)을 사용
