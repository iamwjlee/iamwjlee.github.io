...
3. 번개창을 통한 `감전`-> 천둥의전령발동->전령의 `전기처형보조젬` 효과로 전기처형발동 -> 전투격분효과로 격분수급 -> 번개창강화+무기주입활성화
4. 감전이 잘 안터지면 셀프캐스팅 `저격수의징표` 사용 번개창에 `전도`를 넣는것은 가성비가 아니다
5. 보스전에는 격분이 수급된 `폭창`을 사용하고  치시전의 `저격수의징표`로 격분수급

3. 갑옷 회피에쉴+정신력
```
190 정신력 / 치시전은 퀄작하면 -10
    맵핑: 무희30 재전령30 천전령30 치시전90
    보스전: 치시전90+삼위일체100

145 정신력: 보스전에서는 2번 슬롯엔 치시전만 켜고 나머지는 다끈다(가성비빌드)
