...
자동 세차권출력하려면 
Lioneye의 세차권탭에서 세차권사용이 체크되어 있고, 거래처관리에서 세차권타입이 1/2가 설정되어 있어야
거래처관리.세차권타입(0,1,2) 미출력(0), 타입1(1),타입2(2)
수동 세차권을 출력하려면, 거래처관리에서 세차권타입이 0으로되어 있고,포인트관리사용이 체크,세차권요청시출력이 체크 되있어야
메뉴에서 세차권발행누르면 할인세차권(NR3),포인트세차권(NR4,포인트차감) 구분하여 발해되게끔 되어 있다.

자동으로 세차권출력이 기본이고, 혹시 몰라,세차권미출력시 메뉴얼로 세차권발급하는 개념으로 이해
Grelwood에서 거래처 코드(01) 등록 하고,Lioney의 거래처관리에서 세차권타입을 1로 설정하고,세차권사용체크하면 된다!
