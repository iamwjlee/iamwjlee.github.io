...
- uSD를 PC에 연결하여 인식후에 아래의 내용을 확인
    ...
    - SDNFC.SYS
    - SDSE.SYS
    - JPOS.SET : 직접 만들어서 기물번호 적어둔다(예, 00240012)

- ktc에서 제공받은 JuUGI_Flash.exe를 실행시키다.
    - 사용자ID에 '1',사용자PS에 '1'을 각각입력후 '발급기능접근'버튼을 누른다.(생략 해도 무방)
    - 'uSD연결' 버튼을 누른다
    - 'DV인증서' 버튼옆에 있는 '읽기' 버튼을 누른다.
    - 파일선택창에서 **적절한 인증서**를 선택한다.
        - jpos3000인경우 인증서:[2019_04_30_10_49_10][[KTC01_JWData_010016_900101-KTC01_JWData_010016_900101]]certs.dat
...
B. 펌웨어 유지보수
    위와 같이 하드웨어 구성이 되었으면  
        ...
        2. 가상머신(VMware)에서 장위포스시리얼통신버전(Z:\vm-win7shared\ZJPOSCOMM_SD_V1.0)를 열고 디스펜서와 포스와의 시리얼통신을 확인한다.
            - 포스설정프로그램의 콤포트와 보레이트(19200)확인,JPOS3000의 보레이트확인 
            - 또는 윈도11에서 D:\wj\vm-win7shared\ZJPOSCOMM_SD_V1.0 직접실행해도
        3. 디스펜서의 펌웨어 로그내용을 보려면 '업보드'에 시리얼통신라인을 연결후 테라텀으로 확인한다
...
6. KTC uSD 기물추가및 등록및 인증서 갱신
    ...
    4. uSD카드에 인증서를 쓰기 위해서는 JuUGi_Flash.exe를 사용한다	
        ...
        - JuUGi_Flash.exe실행후 사용자ID와 사용자PS에 1을 입력후 '발급기능 접근' 버튼을 누른다
        - 'uSD 연결' 버튼을 누른다.(연결실패시 'SDGetList 수행실패'라는 로그가 나타난다 )
        - 'DV 인증서(NEW) 아래에 있는 '읽기'버튼을 누른다. 여기서 적절한 인증서(\\jposnas\AS\프로그램_KIOSK\JPOS5000 uSD Card KTC인증서) 를 선택해주어야 한다.  
            (example) [2019_04_30_10_49_10][[KTC01_JWData_010016_900101-KTC01_JWData_010016_900101]]certs.dat
        - '갱신'버튼을 눌러 인증서를 갱신한다('인증서 갱신 성공' 이라는 문구가 나와야 한다)
